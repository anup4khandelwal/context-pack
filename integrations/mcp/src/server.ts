...
function loadRulesWithoutEnv(): string {
...
function sanitizeBundle(bundleJson: any) {
  const filteredFiles = (bundleJson.files || []).filter((file: any) => !String(file.path).includes(".env"));
  return {
    ...bundleJson,
...
function filterMarkdownSections(md: string) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  const budget = Math.min(Math.max(input.budget ?? 14000, MIN_BUDGET), MAX_BUDGET);
  ...
  const cliCandidates = [
    ...
    { cmd: "npx", args: ["-y", "context-pack", ...cliArgs] },
  ];
